	...
	while true do
		...
		if #(coords - Config.CircleZones.WeedProcessing.coords) < 1 then
			...
			if IsControlJustReleased(0, 38) and not isProcessing then
				ESX.TriggerServerCallback('bpt_drugs:cannabis_count', function(xCannabis)
  ...
  local timeLeft = (Config.Delays.WeedProcessing * xCannabis) / 1000
	...
	while timeLeft > 0 do
		...
		if #(GetEntityCoords(playerPed) - Config.CircleZones.WeedProcessing.coords) > 4 then
			...
			TriggerServerEvent('bpt_drugs:cancelProcessing')
			TriggerServerEvent('bpt_drugs:outofbound')
			break
		end
	end
