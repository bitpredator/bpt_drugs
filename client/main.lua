...
function OpenDrugShop()
	local elements = {
		{unselectable = true, icon = "fas fa-cannabis", title = _U('dealer_title')}
	}
	menuOpen = true
	...
	for k, v in pairs(ESX.GetPlayerData().inventory) do
		...
		if price and v.count > 0 then
			elements[#elements+1] = {
				...
				title = ('%s - <span style="color:green;">%s</span>'):format(v.label, _U('dealer_item', ESX.Math.GroupDigits(price))),
				name = v.name,
				price = price,
			}
		end
	...
	ESX.OpenContext("right", elements, function(menu,element)
		...
		ESX.OpenContext("right", elements2, function(menu2,element2)
			...
			ESX.CloseContext()
			TriggerServerEvent('bpt_drugs:sellDrug', element.name, amount)
		end, function(menu)
			menuOpen = false
		end)
	end, function(menu)
